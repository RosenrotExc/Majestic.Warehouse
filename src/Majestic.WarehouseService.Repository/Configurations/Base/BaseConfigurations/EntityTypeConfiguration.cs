...
{
    public class EntityTypeConfiguration<TBaseEntityType> : BaseConfiguration<TBaseEntityType> where TBaseEntityType : BaseEntityType
    {
        public override void Configure(EntityTypeBuilder<TBaseEntityType> builder)
        {
            builder.HasKey((TBaseEntityType x) => x.Id);
            builder.Property((TBaseEntityType x) => x.Id).ValueGeneratedNever();
